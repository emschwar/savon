  ...
  describe '#to_xml' do
    ...
    context 'with a body' do
      ...
      before do
        ...
        signature.use_utc = false
        signature.now(true) # reload @now so it is in local time
      end

      context 'using UTC timestamps' do
        before do
          signature.use_utc = true
          signature.now(true) # reload @now so it is in UTC
        end
        it "should add the timestamp in UTC" do
          signature.to_xml.should match(%r{<wsu:Created>#{Savon::SOAP::DateTimeRegexp}Z</wsu:Created>})
        end
      end
      ...
      it "should add the timestamp" do
        signature.to_xml.should match(%r{<wsu:Timestamp wsu:Id="Timestamp-[^"]+" xmlns:wsu="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd"><wsu:Created>#{Savon::SOAP::DateTimeRegexp}[-+]\d{2}:\d{2}</wsu:Created><wsu:Expires>#{Savon::SOAP::DateTimeRegexp}[-+]\d{2}:\d{2}</wsu:Expires></wsu:Timestamp>})
      end
